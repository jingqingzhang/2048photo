...
import com.yuzhiyun.growup2048.playMusic;
...
//		setBackgroundColor(Color.GREEN);
	...
	private void action(ACTION action) {
		...
		for (int i = 0; i < mColumn; i++) {
			...
			/*
			 * 第二个for 的作用是 获取一行或者一列（如果左右滑动，就是一行，如果上下滑动，就是一列）的所有不是0的item
			 */
	...
	/**
	 * 合并相同的Item
	 * 保存在List<Game2048Item> row里面，然后返回原程序，把数据填充
	 * @param row
	 */
...
//				item1.setNumber(val);
				row.get(j).setNumber(val);
				// 加分
				mScore += val;
				if (mGame2048Listener != null) {
					mGame2048Listener.onScoreChange(mScore);
				}
				// 只要有合并，就把所有后边的方格 向前移动
